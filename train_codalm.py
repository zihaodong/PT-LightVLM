...
import torch
import argparse
from dataset.codalm_dataset import CODALMDataset, get_data_root
...
def custom_train(train_loss, val_loss, epochs, learning_rate):
    ...
    for epoch in range(epochs, config.epochs):
        ...
        epoch_val_loss = val_model(test_dataloader, model)
        ...
        print('Test Loss: ' + str(epoch_val_loss))
        print('---------------------------------------------')
...
def save_experiment(statistics):
    ...
    trial_dict.to_csv(os.path.join('multi_frame_results', config.run_name, 'multi_frame_results.csv'), index=False,
                      header=True)


def get_dataloader(data_dir):
    train_dset = CODALMDataset(
        data_dir=os.path.join(data_dir, 'CODA-LM', 'Train', 'vqa_anno'),
        tokenizer=processor,
    ...
    test_dset = CODALMDataset(
        data_dir=os.path.join(data_dir, 'CODA-LM', 'Mini', 'vqa_anno'),
        tokenizer=processor,
        transform=transforms.Compose([
            transforms.Resize(img_sz),
            transforms.Normalize((127.5, 127.5, 127.5), (127.5, 127.5, 127.5))
...
if __name__ == '__main__':
    ...
    train_dataloader, test_dataloader = get_dataloader(data_root)
    ...
    statistics = [min_train_loss, min_val_loss]
