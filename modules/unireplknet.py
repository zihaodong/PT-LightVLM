...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias,
    ...
    if attempt_use_lk_impl and need_large_impl:
        ...
        try:
            # If you do not install this implementation,
            # you can still use our model anywhere you wish but it will be a bit slower.
...
class SEBlock(nn.Module):
    ...
    def forward(self, inputs, weights=None):
        ...
        if weights is not None:
            bs, num_view, C = weights.shape
            x = x.view(bs, num_view, self.input_channels) + weights.view(bs, num_view, C)
            x = x.view(bs * num_view, self.input_channels)
...
class UniRepLKNetBlock(nn.Module):
    ...
    def compute_residual(self, x, text_feat=None):
        if text_feat is not None:
            y = self.se(self.norm(self.dwconv(x)), text_feat)
        else:
    ...
    def forward(self, inputs, text_feat=None):
        def _f(x, text_feat=None):
            return x + self.compute_residual(x, text_feat)
        if self.with_cp and inputs.requires_grad:
            out = checkpoint.checkpoint(_f, inputs, text_feat)
        else:
            out = _f(inputs, text_feat)
        return out
        ...
        for i in range(4):
            main_stage = nn.Sequential(
                *[UniRepLKNetBlock(dim=dims[i],
                                   ...
                                   attempt_use_lk_impl=attempt_use_lk_impl,
                                   with_cp=with_cp,
                                   use_sync_bn=use_sync_bn) for j in range(depths[i])])
    ...
    def forward(self, x, text_feat=None):
        if self.output_mode == 'logits':
            for stage_idx in range(4):
                ...
                for k, block in enumerate(self.stages[stage_idx]):
                    film = getattr(block, 'film', None)
                    if film is not None:
                        x = block(x, text_feat[stage_idx][k])
                    else:
                        x = block(x)
            # final_feature_maps = x.clone()
